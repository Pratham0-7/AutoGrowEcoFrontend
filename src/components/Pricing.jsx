import { useEffect, useRef, useState } from "react";
import { useNavigate } from "react-router-dom";

const useInView = (threshold = 0.12) => {
  const ref = useRef(null);
  const [inView, setInView] = useState(false);
  useEffect(() => {
    const observer = new IntersectionObserver(
      ([entry]) => { if (entry.isIntersecting) setInView(true); },
      { threshold }
    );
    if (ref.current) observer.observe(ref.current);
    return () => observer.disconnect();
  }, [threshold]);
  return [ref, inView];
};

const CheckIcon = () => (
  <svg className="h-4 w-4 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
    <path strokeLinecap="round" strokeLinejoin="round" d="M4.5 12.75l6 6 9-13.5" />
  </svg>
);

const plans = [
  {
    name: "Solo",
    price: "$199",
    foundingPrice: "$119",
    period: "/month",
    commitment: "3 month minimum",
    tagline: "For small teams just getting started with follow-up automation.",
    popular: false,
    accent: "teal",
    features: [
      "Up to 500 active follow-up contacts",
      "2 users",
      "1 active sequence",
      "Pre-built follow-up templates",
      "AI message generation",
      "Email + SMS",
      "Basic dashboard",
      "Standard support",
    ],
  },
  {
    name: "Team",
    price: "$499",
    foundingPrice: "$299",
    period: "/month",
    commitment: "3 month minimum",
    tagline: "For growing teams who want full control over their follow-up pipeline.",
    popular: true,
    accent: "teal",
    features: [
      "Up to 2,000 active follow-up contacts",
      "5 users",
      "5 active sequences",
      "Pre-built + customizable templates",
      "AI message generation",
      "Email + SMS",
      "Full pipeline dashboard",
      "Sequence analytics",
      "Priority support",
    ],
  },
  {
    name: "Agency",
    price: "$999",
    foundingPrice: "$599",
    period: "/month",
    commitment: "6 month minimum",
    tagline: "For agencies managing multiple clients and high lead volumes.",
    popular: false,
    accent: "blue",
    features: [
      "Up to 5,000 active follow-up contacts",
      "15 users",
      "Unlimited sequences",
      "Full template access + build your own",
      "AI message generation",
      "Email + SMS",
      "Multi-client dashboard",
      "Advanced analytics",
      "Dedicated support",
    ],
  },
];

const accentMap = {
  teal: {
    badge: "border-[#0F5E6E]/20 bg-[#0F5E6E]/5 text-[#0F5E6E]",
    check: "text-[#0F5E6E]",
    border: "border-[#0F5E6E]/30",
    popularBg: "border-[#0F5E6E]/20 bg-[#0F5E6E]/5",
    cta: "bg-[#E8563A] hover:bg-[#d14b30] shadow-[#E8563A]/20",
    ctaOutline: "border-[#0F5E6E]/25 text-[#0F5E6E] hover:bg-[#0F5E6E]/5",
  },
  blue: {
    badge: "border-blue-200 bg-blue-50 text-blue-600",
    check: "text-blue-600",
    border: "border-blue-200",
    popularBg: "border-blue-200 bg-blue-50",
    cta: "bg-blue-600 hover:bg-blue-500 shadow-blue-500/20",
    ctaOutline: "border-blue-200 text-blue-600 hover:bg-blue-50",
  },
};

const PlanCard = ({ plan, delay }) => {
  const [ref, inView] = useInView();
  const a = accentMap[plan.accent];

  return (
    <div
      ref={ref}
      style={{
        opacity: inView ? 1 : 0,
        transform: inView ? "translateY(0)" : "translateY(28px)",
        transition: `opacity 0.6s ease ${delay}ms, transform 0.6s ease ${delay}ms`,
      }}
      className={`relative flex flex-col rounded-2xl border p-7 shadow-sm ${
        plan.popular
          ? `${a.popularBg}`
          : "border-slate-200 bg-white"
      }`}
    >
      {/* Popular badge */}
      {plan.popular && (
        <div className="absolute -top-3.5 left-1/2 -translate-x-1/2">
          <span className="inline-flex items-center gap-1.5 rounded-full border border-[#E8563A]/20 bg-[#E8563A] px-3 py-1 text-xs font-semibold text-white shadow-lg shadow-[#E8563A]/20">
            <svg className="h-3 w-3" fill="currentColor" viewBox="0 0 20 20">
              <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
            </svg>
            Most Popular
          </span>
        </div>
      )}

      {/* Plan name */}
      <p className="text-sm font-semibold uppercase tracking-widest text-[#6B7280]">{plan.name}</p>

      {/* Price */}
      <p className="mt-3 text-xs font-medium uppercase tracking-widest text-[#6B7280]">Starting at</p>
      <div className="mt-1 flex items-end gap-2">
        <span className="text-5xl font-black text-[#1A2E35]">{plan.foundingPrice}</span>
        <div className="mb-1.5 flex flex-col items-start">
          <span className="text-sm font-bold text-[#6B7280] line-through">{plan.price}</span>
          <span className="text-base text-[#6B7280]">{plan.period}</span>
        </div>
      </div>
      <p className="mt-1 text-xs text-[#6B7280]">{plan.commitment}</p>

      {/* Tagline */}
      <p className="mt-4 text-sm leading-relaxed text-[#6B7280]">{plan.tagline}</p>

      {/* Divider */}
      <div className="my-6 h-px bg-slate-200" />

      {/* Features */}
      <ul className="flex flex-col gap-2.5">
        {plan.features.map((f, i) => (
          <li key={i} className={`flex items-start gap-2.5 text-sm ${a.check}`}>
            <span className={`mt-0.5 ${a.check}`}><CheckIcon /></span>
            <span className="text-[#1A2E35]">{f}</span>
          </li>
        ))}
      </ul>

      {/* CTA */}
      <button
        onClick={() => navigate("/schedule")}
        className={`mt-8 w-full rounded-xl px-5 py-3 text-sm font-semibold text-white transition-all duration-200 shadow-lg ${
          plan.popular
            ? `${a.cta}`
            : `border ${a.ctaOutline} bg-transparent`
        }`}
      >
        Book a Demo
      </button>
    </div>
  );
};

const Pricing = () => {
  const navigate = useNavigate();
  const [headerRef, headerInView] = useInView(0.2);
  const [addonRef, addonInView] = useInView(0.2);
  const [noteRef, noteInView] = useInView(0.2);

  return (
    <section className="relative bg-[#FFFBF5] py-24 sm:py-32">
      {/* Top divider */}
      <div className="absolute top-0 left-0 right-0 h-px bg-linear-to-r from-transparent via-slate-200 to-transparent" />

      {/* Background glows */}
      <div className="pointer-events-none absolute inset-0 overflow-hidden">
        <div className="absolute left-1/2 top-0 h-96 w-96 -translate-x-1/2 rounded-full bg-[#0F5E6E]/5 blur-3xl" />
        <div className="absolute right-0 bottom-1/3 h-72 w-72 rounded-full bg-blue-500/5 blur-3xl" />
      </div>

      <div className="relative mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">

        {/* Header */}
        <div
          ref={headerRef}
          style={{
            opacity: headerInView ? 1 : 0,
            transform: headerInView ? "translateY(0)" : "translateY(24px)",
            transition: "opacity 0.7s ease, transform 0.7s ease",
          }}
          className="mx-auto max-w-2xl text-center"
        >
          <div className="inline-flex items-center gap-2 rounded-full border border-[#0F5E6E]/20 bg-[#0F5E6E]/5 px-4 py-1.5 text-sm font-medium text-[#0F5E6E]">
            <svg className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M12 6v12m-3-2.818l.879.659c1.171.879 3.07.879 4.242 0 1.172-.879 1.172-2.303 0-3.182C13.536 12.219 12.768 12 12 12c-.725 0-1.45-.22-2.003-.659-1.106-.879-1.106-2.303 0-3.182s2.9-.879 4.006 0l.415.33M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
            Pricing
          </div>

          <h2 className="mt-5 text-4xl font-bold tracking-tight text-[#1A2E35] sm:text-5xl">
            Simple pricing.{" "}
            <span className="text-[#0F5E6E]">No surprises.</span>
          </h2>

          <p className="mt-4 text-base leading-relaxed text-[#6B7280]">
            Every plan includes a full onboarding session. We handle the setup. You focus on closing.
          </p>
        </div>

        {/* Early Access Banner */}
        <div className="mt-10 flex items-center justify-center gap-3 rounded-xl border border-amber-200 bg-amber-50 px-5 py-3.5 text-sm text-amber-700">
          <svg className="h-4 w-4 shrink-0 text-amber-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
            <path strokeLinecap="round" strokeLinejoin="round" d="M16.5 10.5V6.75a4.5 4.5 0 10-9 0v3.75m-.75 11.25h10.5a2.25 2.25 0 002.25-2.25v-6.75a2.25 2.25 0 00-2.25-2.25H6.75a2.25 2.25 0 00-2.25 2.25v6.75a2.25 2.25 0 002.25 2.25z" />
          </svg>
          <span>
            <span className="font-semibold text-amber-700">Early Access</span>
            {" — "}Join as a founding client and lock in 40% off your first 3 months.{" "}
            <span className="text-amber-500">Limited spots available.</span>
          </span>
        </div>

        {/* Plan cards */}
        <div className="mt-8 grid gap-6 sm:grid-cols-3">
          {plans.map((plan, i) => (
            <PlanCard key={plan.name} plan={plan} delay={i * 100} />
          ))}
        </div>

        {/* WhatsApp Add-on */}
        <div
          ref={addonRef}
          style={{
            opacity: addonInView ? 1 : 0,
            transform: addonInView ? "translateY(0)" : "translateY(24px)",
            transition: "opacity 0.6s ease 0.1s, transform 0.6s ease 0.1s",
          }}
          className="mt-8 overflow-hidden rounded-2xl border border-emerald-200 bg-emerald-50"
        >
          <div className="flex flex-col items-start gap-5 px-7 py-6 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex items-start gap-4">
              {/* WhatsApp icon */}
              <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl border border-emerald-200 bg-white text-emerald-600">
                <svg className="h-5 w-5" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
                </svg>
              </div>
              <div>
                <p className="font-semibold text-[#1A2E35]">WhatsApp Add-on</p>
                <p className="mt-1 text-sm leading-relaxed text-[#6B7280]">
                  Add WhatsApp to any plan and follow up on the most personal channel your leads use.{" "}
                  <span className="text-[#6B7280]/70">Available on all plans.</span>
                </p>
              </div>
            </div>

            <div className="flex shrink-0 items-center gap-4">
              <div className="text-right">
                <p className="text-3xl font-black text-[#1A2E35]">$35</p>
                <p className="text-xs text-[#6B7280]">/month</p>
              </div>
              <button
                onClick={() => navigate("/schedule")}
                className="rounded-xl border border-emerald-300 bg-white px-5 py-2.5 text-sm font-semibold text-emerald-700 transition hover:bg-emerald-50"
              >
                Add to Plan
              </button>
            </div>
          </div>
        </div>

        {/* Fine print */}
        <div
          ref={noteRef}
          style={{
            opacity: noteInView ? 1 : 0,
            transition: "opacity 0.6s ease 0.2s",
          }}
          className="mt-8 text-center text-xs leading-relaxed text-[#6B7280]"
        >
          All plans include a 10-day onboarding session where we handle DLT registration, channel setup, and sequence configuration.{" "}
          Pricing shown in USD.{" "}
          <button
            onClick={() => navigate("/schedule")}
            className="text-[#0F5E6E] underline underline-offset-2 transition hover:text-[#0F5E6E]/70"
          >
            Book a demo
          </button>{" "}
          to discuss your specific requirements and volume.
        </div>

      </div>
    </section>
  );
};

export default Pricing;
