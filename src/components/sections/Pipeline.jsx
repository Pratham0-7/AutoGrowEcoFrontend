...
const PipelineCard = ({ lead, onClick }) => (
  <div onClick={() => onClick(lead)} className="pipeline-card"
    style={{ background: "#070D16", border: "1px solid #1E293B", borderRadius: 10, padding: 12, cursor: "pointer", marginBottom: 8 }}>
...
const PipelineColumn = ({ title, color, bg, leads, onLeadClick }) => (
  <div style={{ background: "#0A0F1C", border: "1px solid #1E293B", borderRadius: 14, padding: 14, minWidth: 220, flex: 1 }}>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <div style={{ display: "flex", gap: 14, overflowX: "auto", paddingBottom: 8 }}>
        <PipelineColumn title="New"        color="#64748B" bg="#1E293B" leads={pipeline.new}        onLeadClick={onSelectLead} />
        <PipelineColumn title="Contacted"  color="#60A5FA" bg="#1E3A5F" leads={pipeline.contacted}  onLeadClick={onSelectLead} />
        <PipelineColumn title="Interested" color="#4ADE80" bg="#052E16" leads={pipeline.interested} onLeadClick={onSelectLead} />
        <PipelineColumn title="Declined"   color="#F87171" bg="#2D0A0A" leads={pipeline.declined}   onLeadClick={onSelectLead} />
      </div>
