...
const Contacts = ({ leads, leadSchedules, updateLeadSchedule, onSelectLead, onStartFollowup, onMarkIndividual, onGoToImport, currentPage, setCurrentPage }) => {
  ...
  const clearFilters = () => { setFilterSendStatus("all"); setFilterReplyStatus("all"); setSearchQuery(""); setCurrentPage(1); };
    ...
    <div style={{ background: "#0D1117", border: "1px solid #1E293B", borderRadius: 16, overflow: "hidden" }}>
      ...
      <div style={{ padding: "14px 20px", borderBottom: "1px solid #1E293B", display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap", justifyContent: "space-between" }}>
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
          ...
          <select value={filterSendStatus} onChange={(e) => { setFilterSendStatus(e.target.value); setCurrentPage(1); }}
          ...
          <select value={filterReplyStatus} onChange={(e) => { setFilterReplyStatus(e.target.value); setCurrentPage(1); }}
            className="crm-input" style={{ borderRadius: 9, padding: "7px 12px", fontSize: 12 }}>
          ...
          <button onClick={onGoToImport} className="crm-btn-primary" style={{ padding: "7px 14px", fontSize: 12 }}>
      ...
      {leads.length === 0 ? (
        <div style={{ padding: "64px 24px", textAlign: "center" }}>
          ...
          <button onClick={onGoToImport} className="crm-btn-primary">Import Contacts</button>
        </div>
      ) : (
        <>
              ...
              <tbody>
                {paginated.map((lead, idx) => (
                  <tr key={lead._id} className="lead-row"
                    ...
                    style={{ borderBottom: "1px solid #0A0F1C", opacity: lead.is_individual_followup ? 0.45 : 1 }}>
                    ...
                    <td style={{ padding: "12px 12px" }} onClick={(e) => e.stopPropagation()}>
                      {lead.is_individual_followup ? (
                        <button title="In Individual pool"
                          style={{ background: "#1C1505", border: "1px solid #D9770644", color: "#FCD34D", borderRadius: 8, width: 30, height: 30, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>
                          <Icon d={ICONS.star} size={12} />
                        </button>
                      ) : (
                        <button onClick={() => onMarkIndividual(lead._id, true)} title="Add to Individual"
